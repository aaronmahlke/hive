...
<template>
  <div class="border-neutral rounded-md border p-2">
      ...
      <div class="min-w-0 flex-1">
        ...
        >
          <button
            ...
            :key="opt"
            type="button"
            class="bg-subtle hover:bg-subtle text-copy text-primary rounded px-2 py-1 text-left"
            :disabled="resolving"
        ...
        <div v-if="!signal.options?.length && !signal.resolved" class="mt-2">
          <form @submit.prevent="submitAnswer" class="flex gap-1">
            <input
              v-model="answer"
              class="bg-subtle border-neutral text-copy text-primary h-6 flex-1 rounded border px-2 outline-none"
